...
func shortLog*(v: FinalityCheckpoints): auto {.raises: [].} =
  (
...
func is_active_validator*(validator: Validator, epoch: Epoch): bool {.raises: [].} =
  ## Check if ``validator`` is active
  validator.activation_epoch <= epoch and epoch < validator.exit_epoch

func is_exited_validator*(validator: Validator, epoch: Epoch): bool {.raises: [].} =
  ## Check if ``validator`` is exited
  validator.exit_epoch <= epoch

func is_withdrawable_validator*(validator: Validator, epoch: Epoch): bool {.raises: [].} =
...
func get_active_validator_indices_len*(
    state: ForkedHashedBeaconState; epoch: Epoch): uint64 {.raises: [].} =
  withState(state):
...
func get_current_epoch*(state: ForkyBeaconState): Epoch {.raises: [].} =
  ## Return the current epoch.
  state.slot.epoch

# https://github.com/ethereum/consensus-specs/blob/v1.2.0-rc.1/specs/phase0/beacon-chain.md#get_current_epoch
func get_current_epoch*(state: ForkedHashedBeaconState): Epoch {.raises: [].} =
  ## Return the current epoch.
...
func get_previous_epoch*(
    state: ForkyBeaconState | ForkedHashedBeaconState): Epoch {.raises: [].} =
...
func get_randao_mix*(state: ForkyBeaconState, epoch: Epoch): Eth2Digest {.raises: [].} =
  ## Returns the randao mix at a recent ``epoch``.
  state.randao_mixes[epoch mod EPOCHS_PER_HISTORICAL_VECTOR]

func bytes_to_uint64*(data: openArray[byte]): uint64 {.raises: [].} =
  doAssert data.len == 8

  # Little-endian data representation
  uint64.fromBytesLE(data)

func uint_to_bytes*(x: uint64): array[8, byte] {.raises: [].} = toBytesLE(x)
func uint_to_bytes*(x: uint32): array[4, byte] {.raises: [].} = toBytesLE(x)
func uint_to_bytes*(x: uint16): array[2, byte] {.raises: [].} = toBytesLE(x)
func uint_to_bytes*(x: uint8): array[1, byte] {.raises: [].} = toBytesLE(x)

# https://github.com/ethereum/consensus-specs/blob/v1.2.0-rc.1/specs/phase0/beacon-chain.md#compute_domain
func compute_domain*(
    domain_type: DomainType,
    fork_version: Version,
    genesis_validators_root: Eth2Digest = ZERO_HASH): Eth2Domain {.raises: [].} =
...
func get_domain*(
    ...
    domain_type: DomainType,
    epoch: Epoch,
    genesis_validators_root: Eth2Digest): Eth2Domain {.raises: [].} =
...
func compute_signing_root*(ssz_object: auto, domain: Eth2Domain): Eth2Digest {.raises: [].} =
...
func add_flag*(flags: ParticipationFlags, flag_index: int): ParticipationFlags {.raises: [].} =
  let flag = ParticipationFlags(1'u8 shl flag_index)
  flags or flag

# https://github.com/ethereum/consensus-specs/blob/v1.2.0-rc.1/specs/altair/beacon-chain.md#has_flag
func has_flag*(flags: ParticipationFlags, flag_index: int): bool {.raises: [].} =
  let flag = ParticipationFlags(1'u8 shl flag_index)
  (flags and flag) == flag
...
func get_safety_threshold*(store: LightClientStore): uint64 {.raises: [].} =
  max(
...
func toMeta*(update: SomeLightClientUpdate): LightClientUpdateMetadata {.raises: [].} =
...
func is_better_data*(new_meta, old_meta: LightClientUpdateMetadata): bool {.raises: [].} =
...
func is_merge_transition_complete*(state: bellatrix.BeaconState): bool {.raises: [].} =
...
func is_execution_block*(blck: SomeForkyBeaconBlock): bool {.raises: [].} =
...
func is_merge_transition_block(
    ...
    body: bellatrix.BeaconBlockBody | bellatrix.TrustedBeaconBlockBody |
          bellatrix.SigVerifiedBeaconBlockBody): bool {.raises: [].} =
...
func compute_timestamp_at_slot*(state: ForkyBeaconState, slot: Slot): uint64 {.raises: [].} =
...
func build_empty_execution_payload*(state: bellatrix.BeaconState): ExecutionPayload {.raises: [].} =
